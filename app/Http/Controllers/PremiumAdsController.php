...
class PremiumAdsController extends Controller {
	...
	public function startPayment(Request $request, $ads_id, $model) {

		$id = base64_decode($ads_id);
		$model1 = base64_decode($model);
		$model_name = 'App\\' . $model1;
		$data['id'] = $id;
		$data['model'] = $model1;
		$data['item'] = $model_name::where('id', $id)->first();
		return view('front.paypal.premiumads', $data);
	}
	...
	public function fun_notify_url(Request $request, $id, $model) {
		//$post = $request->all();
		$post = $_POST;
		// \file_put_contents("testpost".time().".txt", \json_encode($post));
		...
		if ((strcmp($response, 'VERIFIED') == 0 || strcmp($response, 'UNVERIFIED') == 0) && $post['payment_status'] == "Completed") {
			...
			// \file_put_contents("testpayment".time().".txt", \json_encode($payment));
	...
	public function payment(Request $request) {
		...
		$paypalConfig = [
			...
			'notify_url' => route('ads_purches.pay'),
		];

		$paypalUrl = $enableSandbox ? 'https://www.sandbox.paypal.com/cgi-bin/webscr' : 'https://www.paypal.com/cgi-bin/webscr';

		// Product being purchased.
		$itemName = $request->item_number == 'Paid ads';
		$itemAmount = $request->item_number == '$5';
		...
		if (!isset($request->txn_id) && !isset($request->txn_type)) {
			...
			$data = [
				'item_name' => $request->item_number == 'Paid ads',
				'item_number' => "Paid Advertice",
				'payment_status' => $request->payment_status,
				'payment_amount' => $request->item_number == '$5',
