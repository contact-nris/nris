            ...
            <div class="row align-items-center">
                <div class="col-lg-2 col-md-12 text-center text-lg-left">
                    <div class="breadcrumb-wrapper">
                        <h2 class="product-title">Pubs</h2>

                        <p><a href="{{ route('home') }}" class="text-white" style="font-size: 17px;">Home /</a>
                            <span
                                style="font-size:17px ;color:var(--main-color)">Theater</span>
                                 </p>
                    </div>
                </div>
                <div class="col-lg-10 col-md-12 text-center text-lg-right">
                            ...
                            <div class="form-group inputwithicon w-25">
                                ...
                                <div class="select">
                                    <select name="theater_type">
                                        ...
                                        <option
                                            <?=$theater_type == $t->slug || request()->theater_type == $t->slug ? 'selected' : ''?>
                                            value="<?=$t->slug?>">
                                            <?=$t->type?>
                                        </option>
                                        <?php }?>
	...
	?>
                                <div class="col-xs-12 col-sm-12 col-md-2 col-xl-2 p-0">
                                    <div class="featured-box">
                                        <figure>
                                            <a href="{{ route('front.theaters.view', $list->slug) }}"><img
                                                    class="img-fluid " src="{{ $list->image_url }}" alt=""></a>
                                        </figure>
                                        <?php
$hour = App\BusinessHour::where(['model' => 'f_theater', 'model_id' => $list->id])
		->get()
		->first();
	if ($hour) {
	...
	?>
                                        ...
                                        <div class="feature-content">
                                            <h4><a
                                                    href="{{ route('front.theaters.view', $list->slug) }}">{{ $list->name }}</a>
                                            </h4>
                                            <div class="listing-bottom">
                                                <div class="d-flex justify-content-between">
                                                    <div>City: {{ $list->city_name }}</div>
                                                    <div class="text-center"><i class="fa fa-comment-o"></i>
                                                        <div>{{ count($list->comments) }}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
$('#form_ajax').on('submit', function(e) {
    var url = "{{ route('front.theaters.list') }}";
    var theater_type = $('select[name=\'theater_type\']').val();
    if (theater_type != '') {
        url += '/' + encodeURIComponent(theater_type);
    }
