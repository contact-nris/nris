            ...
            <div class="row align-items-center">
                <div class="col-lg-7 col-md-12 text-center text-lg-left">
                    <div class="breadcrumb-wrapper">
                        <h2 class="product-title">RealEstate</h2>

                        <p><a href="{{ route('home') }}" class="text-white" style="font-size: 17px;">Home /</a>
                            <span
                                style="font-size:17px ;color:var(--main-color)">RealEstate</span>
                                 </p>
                    </div>
                </div>
                    ...
                    <form method="get" id="form_ajax" class="row" >
                        ...
                        <div class=" col-lg-3 col-md-3 col-xs-3">
                            <select name="category_type" class="form-control">
                                ...
                                <option <?=$t->slug == $category_type ? 'selected' : ''?> value="<?=$t->slug?>">
                                    <?=$t->name?>
                                </option>
                                <?php }?>
                            </select>
                                    ...
                                    <?php foreach ($lists as $key => $list) {?>
                                        ...
                                        <div class="featured-box">
                                            <figure class="update position-relative">
                                                <a href="{{ route('realestate.view', $list->slug ) }}">
                                                    ...
                                                    <?php } else {?>
                                                        <img  onerror="this.onerror=null;this.src='https://nris.com/stuff/images/default.png';"  class="img-fluid" src="{{ asset('upload/commen_img/real-estate.png') }}" alt="">
                                                    <?php }?>
                                                </a>
                                                @if($list->images)
                                                        ...
                                                        @foreach($list->images as $image)
                                                            <div class="item">
                                                                <a target="_blank" class="fancybox" rel="{{ $list->title }}"
                                                                    href="{{ $image }}">
                                                                    <img  onerror="this.onerror=null;this.src='https://nris.com/stuff/images/default.png';"  class="img-fluid" width="50" src="{{ $image }}"
                                                                        alt="{{ $list->title }}">
                                                                </a>
                                                            </div>
                                                        @endforeach
                                            ...
                                            <div class="feature-content update">
                                                <div class="product">{{ $list->category_name }}</div>
                                                <h4>
                                                    <a href="{{ route('realestate.view', $list->slug ) }}">{{ $list->ShortTitle }}</a>
                                                </h4>
                                                <div class="dsc">{{ $list->short_desc }}</div>
                                                <div class="specification">
                                                    <ul class="list-specification">
                                                        <div class="row pl-3">
                                                            <li><i class="fa fa-calendar"></i><strong>Built
                                                                    Year</strong>{{ $list->built_year }}</li>
                                                            <li><i class="fa fa-dollar"></i><strong>Property
                                                                    Price</strong>{{ amount($list->property_price) }}
                                                            </li>
                                                            <li><i class="fa fa-square"></i><strong>Square Feet
                                                                </strong>{{ $list->square_feet }}</li>
                                                            <li><i class="fa fa-bed"></i><strong>Bedrooms
                                                                </strong>{{ $list->bedrooms }}</li>
                                                        </div>
                                                    </ul>
                                                </div>
...
<script>
    $('#form_ajax').on('submit', function (e) {
        var url = "{{ route('realestate.index') }}";
        ...
        if (city_code != '') {
            url.includes('filter_name') ? url += '&city_code=' + encodeURIComponent(city_code) : url +=
                '?city_code=' + encodeURIComponent(city_code);
        }
        location = url;
        e.preventDefault();
    });
