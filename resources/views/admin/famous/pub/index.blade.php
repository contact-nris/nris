...
<div class="my-3 my-md-5">
    <div class="container">
        <div class="page-header">
            <h1 class="page-title">Famous Pubs List</h1>
            <div class="page-options">
                 <a href="{{ route('famous_pubs.form') }}" class="btn btn-sm btn-outline-primary">Add New</a>
            </div>
        </div>
    </div>
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Famous Pubs Details</h3>
                    </div>
                        ...
                        <tbody>
                            @foreach ($lists as $list)
                                <tr>
                                    ...
                                    <td class="text-left">{{ $list->pub_name }}</td>
                                    <td class="text-left">{{ $list->state_code }}</td>
                                    <td class="text-left">{{ $list->city_id }}</td>
                                    <td class="text-left">{{ $list->pubs_type_name }}</td>
                                    <td class="text-left">{!! status_text($list->status) !!} </td>

                                    <td class="text-right">
                                         <a class="btn btn-outline-warning btn-sm"target="_blank" href="{{ route('front.pubs.view',$list->slug) }}"><i class="fe fe-eye"></i></a>

                                        <a class="btn btn-outline-primary btn-sm" href="{{ route('famous_pubs.form',[$list->id]) }}"><i class="fe fe-edit"></i></a>
                                        <a class="btn btn-outline-danger confirm-link btn-sm" href="{{ route('famous_pubs.delete',$list->id) }}"><i class="fe fe-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
