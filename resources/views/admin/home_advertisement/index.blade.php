...
<div class="my-3 my-md-5">
    <div class="container">
        <div class="page-header">
            <h1 class="page-title">GIF Advertisement List</h1>
            <div class="page-options">
                 <a href="{{ route('home_advertisement.form') }}" class="btn btn-sm btn-outline-primary">Add New</a>
            </div>
        </div>
    </div>
                        ...
                        <tbody>
                            @foreach ($lists as $list)
                                <tr>
                                    ...
                                    <td class="text-left">{{ $list->adv_name }}</td>
                                    <td class="text-left">{{ get_state_name($list->state_id ) }}</td>
                                    <td class="text-left">{{ $list->ad_position }}--{{$list->ad_position_no}}</td>
                                    <td class="text-left">{{ date_f($list->sdate) }}</td>
                                    <td class="text-left">{{ date_f($list->edate) }}</td>
                                    ...
                                    <td class="text-right">
                                        ...
                                        <a class="btn btn-outline-primary btn-sm" href="{{ route('home_advertisement.form',[$list->id]) }}"><i class="fe fe-edit"></i></a>
                                        <a class="btn btn-outline-danger confirm-link btn-sm" href="{{ route('home_advertisement.delete',$list->id) }}"><i class="fe fe-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
