            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Baby Sitting Details</h3>
                    </div>
                    ...
                    <table class="table card-table table-sm table-hover">
                        ...
                        <tbody>
                            ...
                            <tr valign="middle">
                                ...
                                <td class="text-left">{{ substr(strip_tags($list->message),0,50) }}</td>
                                <td class="text-left">{{ $list->category }} </td>
                                ...
                                <td class="text-right">
                                    <div>
                                        ...
                                        <button data-id='<?=$list->id?>' data-name="isPayed" class="btn toggle-btn btn-<?=$list->isPayed == 'Y' ? 'success' : 'secondary'?> btn-sm" type="button">
                                         ...
                                         <a class="btn btn-outline-warning btn-sm"target="_blank" href="{{ route('front.babysitting.view',$list->id) }}"><i class="fe fe-eye"></i></a>

                                        <a class="btn btn-primary btn-sm" href="{{ route('babysitting_classified.view',$list->id) }}"><i class="fe fe-eye"></i></a>
                                        <a class="btn btn-outline-danger confirm-link btn-sm" href="{{ route('auto_classified.delete',$list->id) }}"><i class="fe fe-trash"></i></a>
...
<script type="text/javascript">
    $(".toggle-btn").click(function() {
        $this = $(this);
        $.ajax({
            url: '{{ route("babysitting_classified.change_type") }}',
            type: 'POST',
            dataType: 'json',
            data: {
